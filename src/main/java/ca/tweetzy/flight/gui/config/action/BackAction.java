...
/**
 * Built-in action handler for going back to the parent GUI.
 */
public final class BackAction implements GuiConfigActionHandler {
    ...
    public boolean execute(@NonNull GuiClickEvent event, @NonNull GuiConfigContext context, @NonNull String actionString, @NonNull Map<String, String> parameters) {
        if (event.gui.getParent() != null) {
            event.manager.showGUI(event.player, event.gui.getParent());
        } else {
            event.gui.close();
        }
        return true;
